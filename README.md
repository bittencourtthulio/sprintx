...
<img alt="sprintx — a metade Build do método Expx: seis fases, da ingestão à execução autônoma" src=".github/assets/anim/hero.svg" width="100%">
...
<p>
  ...
  &nbsp;·&nbsp;
  <a href="#as-seis-fases">Fases F1–F6</a>
  &nbsp;·&nbsp;
  <a href="#o-ecossistema-expx">O ecossistema</a>
  &nbsp;·&nbsp;
  <a href="#os-contratos">Contratos</a>
  &nbsp;·&nbsp;
...
Descreva o que quer construir. **Não é preciso dizer `sprintx`, "plano" nem "sprint"** — a skill dispara sozinha ao reconhecer a descrição de algo a ser construído, e leva a feature pelas seis fases até a entrega:

<p align="center">
  <img alt="Demonstração: o usuário descreve a feature e a sprintx a leva pelas seis fases até a entrega" src=".github/assets/anim/demo-terminal.svg" width="100%">
</p>

```bash
npx expxdev init          # instala a sprintx e o resto do método no seu harness
```
...
Pronto. A F2 é o único momento em que a IA vai perguntar alguma coisa. Depois dela, o plano é auditado e executado sem parar — dúvida nova vira bloqueio registrado, nunca uma pergunta.
...
<p align="center">
  <img alt="A skill descobre em que fase está inspecionando o disco, nunca perguntando" src=".github/assets/anim/maquina-de-estados.svg" width="100%">
</p>
...
```mermaid
flowchart TB
    A(["uma ideia"]) --> F1["<b>F1</b> INGESTÃO"]
    F1 -->|"grava base/"| F2["<b>F2</b> DESCOBERTA"]
    F2 -->|"grava 00-DECISOES.md"| F3["<b>F3</b> PLANO"]
    F3 -->|"grava sprint-01/"| F4["<b>F4</b> ORQUESTRADOR"]
    F3 -.->|"só se você pedir"| F35["<b>F3.5</b> ESTIMATIVA<br/>opcional · nunca bloqueia"]
    F35 -.->|"00-ESTIMATIVA.md"| F4
    F4 -->|"grava ORQUESTRADOR.md"| F5["<b>F5</b> AUDITORIA"]
    F5 -->|"VEREDITO: SIM"| F6["<b>F6</b> EXECUÇÃO"]
    F5 -->|"achado de severidade ALTA"| F3
    F6 --> Z(["FECHAMENTO.md<br/>a feature entregue"])

    classDef fase fill:#eceff1,stroke:#78909c,color:#263238
    classDef opc fill:#f5f5f5,stroke:#b0bec5,color:#546e7a,stroke-dasharray:4 3
...
<p align="center">
  <img alt="O ecossistema Expx: prodx decide se há trabalho, sprintx decide como, camadas modificam o comportamento, mergex entrega, buildx orquestra o projeto inteiro" src=".github/assets/anim/ecossistema.svg" width="100%">
</p>
...
<p align="center">
  <img alt="Build e Run: sprintx e runx compartilham os mesmos contratos. Muda o gatilho e o tamanho, nunca o rigor." src=".github/assets/anim/build-run.svg" width="100%">
</p>
...
Toda task nasce com um contrato completo e só morre verde. O teste vem antes do código, os dois testes têm de passar, e não existe "concluído com ressalva":

<p align="center">
  <img alt="O ciclo de uma task: contrato, teste antes do código, vermelho, implementação, os dois testes verdes, e só então concluída" src=".github/assets/anim/tdd-ciclo.svg" width="100%">
</p>
...
### O grafo que o plano desenha sozinho

A F3 grava, dentro de cada `sprint-NN/fases.md`, um bloco Mermaid derivado dos campos `depende_de`, `fase` e `status` das tasks — e a F6 atualiza a cor de cada nó ao fechar a task. É assim que uma sprint aparece no GitHub e no VS Code, sem infraestrutura nenhuma: **o que corre em paralelo** é a ausência de caminho entre dois nós, e **o que trava tudo** é o caminho crítico, em traço grosso.

```mermaid
%% Grafo de tasks — sprint-01 — gerado pela sprintx a partir de tasks.md
flowchart LR
  subgraph fase_01_1["F-01.1 — capacidade de testar"]
    T_01_01["T-01.01<br/>config, client e fixtures"]
    T_01_05["T-01.05<br/>docs do endpoint"]
  end
  subgraph fase_01_2["F-01.2 — exportação"]
    T_01_02["T-01.02<br/>client de exportação CSV"]
    T_01_03["T-01.03<br/>formatação de datas"]
  end
  subgraph fase_01_3["F-01.3 — integração na tela"]
    T_01_04["T-01.04<br/>tela chama o client"]
  end
  T_01_01 --> T_01_02
  T_01_01 --> T_01_03
  T_01_02 --> T_01_04
  ...
  class T_01_05 andamento
  class T_01_03 bloqueada
  class T_01_04 pendente
  class T_01_01,T_01_02,T_01_04 critico
```

Se dois campos se contradizem — `paralelizavel: true` com `depende_de` preenchido, dependência para um id que não existe, ciclo — o diagrama **não é gerado** e a contradição é reportada como erro de plano. O grafo descobre um plano inconsistente antes de a execução descobrir.

---

## Execução autônoma

A F6 lê o `ORQUESTRADOR.md` e implementa até o fim. Só corre em paralelo o que o plano declarou. Dúvida nova vira uma linha em `00-BLOQUEIOS.md`, a task fica `bloqueada`, e o agente pula para a próxima paralelizável — **nunca para, nunca pergunta**:

<p align="center">
  <img alt="F6: a execução nunca para. Paralelismo declarado no plano, dúvida vira bloqueio registrado, e o agente segue para a próxima task." src=".github/assets/anim/execucao-autonoma.svg" width="100%">
</p>

O que acontece, passo a passo, entre o agente, os hooks do harness e os arquivos de estado:

```mermaid
sequenceDiagram
    autonumber
    participant O as ORQUESTRADOR.md
    participant A as agente (F6)
    participant H as hooks (harness)
    participant T as tasks.md
    participant B as 00-BLOQUEIOS.md
    A->>O: lê o mapa: rota, paralelismo, caminho crítico
    loop para cada task na rota
        A->>T: status: em_andamento
        H-->>A: task-reivindicada? (aviso se outra sessão a abriu)
        A->>A: escreve o teste e roda: vermelho
        H-->>A: escopo-da-task · tdd-teste-antes (aviso)
        A->>A: implementa e roda: verde (integração + funcional)
        A->>T: status: concluida · suite: parcial
        H-->>A: task-so-fecha-verde (aviso)
    end
    alt dúvida nova durante a execução
        A->>B: B-NN | task | descrição | o que destravaria
        A->>T: status: bloqueada
        Note over A: pula para a próxima paralelizável — nunca pergunta
    end
    Note over A,H: fechar a sprint: arvore-limpa-antes-da-suite, depois a suíte inteira
    A->>O: FECHAMENTO.md + relatório final (concluídas, bloqueios, saída da suíte)
```
...
| `task-reivindicada` | antes de abrir task | aviso | Não abrir task que outra sessão já abriu |
| `arvore-limpa-antes-da-suite` | antes de rodar a suíte | aviso | Suíte inteira só com a árvore limpa |
...
<p align="center">
  <img alt="Três agentes de veredito, somente leitura: investigador, auditor-plano e revisor-testes. Apontam, não corrigem." src=".github/assets/anim/agentes.svg" width="100%">
</p>
...
<p align="center">
  <img alt="Sessões paralelas: worktree por feature, reivindicação de task pelo rastro e árvore limpa antes da suíte inteira" src=".github/assets/anim/sessoes-paralelas.svg" width="100%">
</p>
...
```mermaid
flowchart LR
    main(["meu-projeto/ · main"])
    subgraph wt1["../meu-projeto-exportacao-csv/ · feature/exportacao-csv · claude@a1"]
        direction LR
        a1["T-02.01"] --> a2["T-02.02"] --> a3["T-02.03"] --> a4{{"fechar sprint<br/>árvore limpa ✓<br/>suíte inteira · verde"}}
    end
    subgraph wt2["../meu-projeto-webhooks/ · feature/webhooks · opencode@b7"]
        direction LR
        b1["T-02.01"] --> b2["T-02.02"] --> b3["T-02.03"]
    end
    main -->|git worktree| wt1
    main -->|git worktree| wt2
    a4 -->|PR pela mergex| main

    classDef ok fill:#d4f4dd,stroke:#2e7d32,color:#1b3d20
    classDef doing fill:#fff3cd,stroke:#b8860b,color:#4a3800
    classDef todo fill:#eceff1,stroke:#78909c,color:#263238
    class a1,a2,a3 ok
    class b1 ok
    class b2 doing
    class b3 todo
...
<p align="center">
  <img alt="F3.5: faixa de esforço, nunca número único; esforço, nunca prazo. Esforço total versus caminho crítico." src=".github/assets/anim/estimativa.svg" width="100%">
</p>
...
Todo arquivo de estado carrega um **frontmatter YAML legível por máquina**, para que um painel de operação leia o andamento das features sem depender de prosa. O painel apenas **lê**; a skill continua sendo a única a escrever. A máquina lê o YAML, a pessoa lê a prosa abaixo dele:
...
docs/sprintx/
  features/
    <slug-da-feature>/
      ...
      FECHAMENTO.md        o que a feature entregou, e onde (gravado ao fim da F6)
      base/
        00-INDICE.md
        00-LACUNAS.md
      ...
      sprint-01/
        sprint.md
        fases.md           inclui o grafo Mermaid das tasks da sprint
        tasks.md
      sprint-02/ ...
...
.claude/
  skills/sprintx/
    ...
    references/
      ...
      09-diagrama.md            o bloco Mermaid do grafo de tasks dentro de fases.md
  ...
  hooks/
    ...
    sprintx/                    escopo-da-task, task-so-fecha-verde, tdd, placeholder, reivindicação, árvore limpa
  agents/
...
.github/assets/                 banner, badges e as animações deste README
...
<a href="https://github.com/bittencourtthulio/prodx">prodx</a></sub>
</div>
